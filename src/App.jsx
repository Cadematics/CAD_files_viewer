...
import { MapControls, OrbitControls, Center, Resize, TransformControls } from '@react-three/drei'
...
function Dxf({ src, font = '/helvetiker_regular.typeface.blob' }) {
  ...
  return (
    <>
      {drawing.children.map((child, index) => (
        <SelectableObject
          key={index}
          object={child}
      ...
      {selected !== null && (
        <TransformControls object={drawing.children[selected]} mode="translate" />
      )}
    </>
  )
}

function SelectableObject({ object, isSelected, onSelect }) {
  const ref = useRef()

  useEffect(() => {
    if (isSelected && ref.current) {
      // Optional: highlight selection with color
